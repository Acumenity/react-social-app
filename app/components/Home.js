...
var Home = React.createClass({
	...
	handlerSubmitPost:function(e){
		...
		this.state.reply.push({"username":this.props.userName,"post":post.post,"text":"","index":post.index})
		this.setState({post: ""});
	},
				...
				if(username == this.props.postList[i].username ){
					 for(var j = 0; j < this.props.postList[i].post.length ; j++ ){
						 if(text == this.props.postList[i].post[j].post){
							  return ({"post":this.props.postList[i].post[j],"index":j});
						 }
					 }
				}
			}
			...
			var reply = [];
			 for(var i=0 ; i < this.props.postList.length; i++){
				 this.props.postList[i].post.forEach(function(post,index) {
					 reply.push({"username":this.props.postList[i].username,"post":post,"text":"","index":index});
		 ...
		 for(var ii=0 ; ii < this.props.postList.length; ii++){
		  ...
		  var username = this.props.postList[ii].username;
				this.props.postList[ii].post.forEach(function(post) {
					rows.push(<PostUI post={post.post} username={username} onReply={this.handerReply} replyList={post.replies}  reply={this.state.reply[index]} onReplyChange={this.handerReplyChange} number={index} likes={post.likes} onSubmitLike={this.handerSubmitlike} ></PostUI>);
					index++;
			}.bind(this));
		}
	}
	return (
				<div>
				<PostBox textValue={this.state.post} onTextChange={this.handerTextAreatInput} onSubmitPost={this.handlerSubmitPost} ></PostBox>
				<div>{rows}</div>
			 </div>
	);
}
...
var mapDispatchToProps = function (dispatch) {
  return {
    addpost: function(post,user){ dispatch(actions.addPost(post,user)); },
    like: function(user,postNumber){ dispatch(actions.likePost(user,postNumber)); },
		reply: function(user,postNumber,text){ dispatch(actions.addReply(user,postNumber,text)); },
  };
}
